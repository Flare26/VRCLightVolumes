...
namespace VRCLightVolumes
{
...
#endif
    {
        ...
        [Tooltip("Minimum lightness value sampled from the Render Texture. Helps to keep some minimum brightness in dark scenes when the color avg of the screen is dark. Affects connected volumes color data, not intensity values.")]
        public float minSampleLightness;
        [Tooltip("Lightness clip threshold. Pixels with lightness below this value will not be clamped to min sample lightness.")]
        public float lightnessClip;
        [Space]
        ...
        private void SetColor()
        {
            ...
            if (v > lightnessClip)
            {
                v = Mathf.Max(v, minSampleLightness);    // set floor here
